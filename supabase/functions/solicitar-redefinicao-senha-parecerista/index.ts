...
serve(async (req) => {
  ...
  try {
    ...
    const { data: parecerista, error: pareceristaError } = await supabase
      .from('pareceristas')
    ...
    if (pareceristaError || !parecerista) {
    ...
    const { data: token, error: tokenError } = await supabase
      .rpc('gerar_token_recuperacao_parecerista', {
    ...
    const resetUrl = `${baseUrl}/${nomePrefeitura}/parecerista/redefinicao-senha?token=${token}&email=${encodeURIComponent(email)}`
    ...
    const emailHtml = `
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            <h2>Olá, ${parecerista.nome}!</h2>
            
            <p>Você solicitou a redefinição de senha da sua conta de parecerista no Portal Cultural.</p>
